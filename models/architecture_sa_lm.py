...
class MainArchitecture(nn.Module):
    def __init__(self, vocab, config, embedd_etype=None):
        ...
        if self.decode_layer == 'lstm':
            ...
            self.extra_linear = nn.Linear(config.hidden_size_tagger*4, config.hidden_size_tagger*2)
    ...
    def compute_loss(self, segmentation, gold_segmentation, segment_mask, nuclear_relation, gold_nuclear_relation, len_golds, depth):    
        ...
        return loss, nuc_rel_loss, seg_loss
    ...
    def prepare_prediction_for_testing(self, encoder_output, cur_span_pairs):
        ...
        if self.config.use_gpu:
            stack_index = stack_index.to(device)
            stack_denominator = stack_denominator.to(device)
    ...
    def decode_training_dynamic_oracle(self, encoder_output, gold_nuclear_relation, 
        ...
        if self.config.use_gpu:
            all_segment_gold = all_segment_gold.to(device)
            all_nuclear_relation_gold = all_nuclear_relation_gold.to(device)
