...
def get_test_loss(network, instances, vocab, config):
    ...
    for i in range(0, total_data, config.batch_size):
        ...
        indices = np.array((range(i, end_index)))
        subset_data = batch_data_variable(instances, indices, vocab, config)
        cost, cost_val, nuc_rel_loss, seg_loss = network.loss(subset_data, None)
...
def main():
    ...
    tb_writer = SummaryWriter(os.path.join(config.model_path,args.experiment))
    ...
    for epoch in range(0, config.max_iter):
        ...
        for i in range(0, total_data, batch_size):
            ...
            tb_writer.add_scalar('1_training/loss',np.mean(costs),epoch)
        ...
        tb_writer.add_scalar('2_evaluating/loss',mean_loss,epoch)
        tb_writer.add_scalar('2_evaluating/nuc_rel_loss',nuc_rel_loss,epoch)
        tb_writer.add_scalar('2_evaluating/seg_loss',seg_loss,epoch)
