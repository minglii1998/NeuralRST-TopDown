...
class Instance(object):
    def __init__(self, total_words, edus, gold_actions, result):
...
class EDU(object):
    def __init__(self, start_index, end_index):
        ...
        self.start_index_word = start_index
        self.end_index_word = end_index
        self.etype = '' # string
    ...
    def tokenize(self, tokenizer, total_words, start_token_idx):
        word_list = total_words[self.start_index_word:self.end_index_word+1]
        # remove edu type
        if word_list[-1] == self.etype:
            word_list = word_list[:-1]
        tokens, length = tokenizer(word_list)
        self.start_index = start_token_idx
        self.end_index = start_token_idx + length - 1 
        self.end_index = self.end_index.item()
        self.token_list = tokens['input_ids'].tolist()[0]

        return tokens, self.token_list, self.end_index
